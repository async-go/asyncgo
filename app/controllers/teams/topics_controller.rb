...
module Teams
  class TopicsController < ApplicationController
    ...
    def show
      @topic = Team.find(params[:team_id]).topics.find(params[:id])
      Pundit.authorize(current_user, @topic, :show?, policy_class: Teams::TopicPolicy)
    end

    def new
      @topic = Team.find(params[:team_id]).topics.build
      Pundit.authorize(current_user, @topic, :new?, policy_class: Teams::TopicPolicy)
    end

    def create
      @topic = Team.find(params[:team_id]).topics.build(topic_params)
